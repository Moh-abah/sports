...
export default function EventCard({ event }: EventCardProps) {
  const isLive = event.status?.toLowerCase() === "in progress"
  const isFinished = event.status?.toLowerCase() === "finished"

  
  const eventId = event.idEvent || "unknown"

  return (
    <Link href={`/event/${eventId}`} className="block" onClick={(e) => {
      console.log("Card dataaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa:", event)
    }}
>
    ...
    ${isLive
          ...
          <div className="flex justify-between items-center mb-4">
            <div className="flex flex-col items-center w-5/12">
              ...
              {event.intHomeScore && (
                <span className="text-2xl font-bold text-blue-600 dark:text-blue-400">{event.intHomeScore}</span>
              )}
            </div>
            ...
            <div className="flex flex-col items-center w-5/12">
              ...
              {event.intAwayScore && (
                <span className="text-2xl font-bold text-blue-600 dark:text-blue-400">{event.intAwayScore}</span>
              )}
            </div>
          </div>
